...
class User{
	...
	public static function getUserInfo($user,$byid=true,$is_json=false,$checkpassword=false,$passwordString=array()){
		...
		// if($byid){
			$q=mysqli_query($mysql,"SELECT user_id as uid, user_profile_photo_id, user_password, user_range, user_email, user_fullname, user_range, user_username, user_country_id, user_bio, (select uploads.upload_file_unique from uploads WHERE uploads.upload_user_id=user_id and uploads.upload_id=user_profile_photo_id) as upload_file_unique, (select count(*) from posts where post_user_id=uid) as songs, (select count(*) from collections where collection_user_id=uid) as collections, youtube_url, website_url, facebook_url, twitter_url, soundcloud_url, vimeo_url FROM user WHERE user.user_id='{$user}' OR user.user_username='{$user}'");
		// }else{
