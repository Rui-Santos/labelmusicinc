...
<div class="choseoptions"><ul>
	<li><a href="{$link}cuenta">Iniciar sesi&oacute;n</a></li>
	<li><a href="{$link}?module=cuenta&registrar">Crear cuenta</a></li>
</ul></div>
</div>
...
<h1><a href="{$link}cuenta">Cuenta</a> <span>Crear cuenta</span></h1>
</div>
...
echo <<<EOD
	...
	<form role="form" class="form-horizontal" action="{$register_link_action}" method="post">
	...
	<input type="hidden" name="_usr_is_fb" value="false" />
		...
		<div class="col-sm-12">
			...
			<select id="_usr_ge" name="_usr_ge" class="form-control">
					...
					{foreach from=$gender item=curr_id}
					  <option value="{$curr_id}">{$curr_id}</option>
					{/foreach}
				</select>
		</div>
		</div>
		...
		<div class="col-sm-12">
			...
			<select id="_usr_ci" name="_usr_ci" class="form-control">
					...
					{foreach from=$country item=curr_id name=country}
					  <option value="{$smarty.foreach.country.iteration-1}">{$curr_id}</option>
					{/foreach}
				</select>
		</div>
		</div>
		...
		<div class="col-sm-12">
			...
			<div class="col-sm-4">
				<select id="_usr_bi-bi_day" name="_usr_bi-bi_day" class="form-control">
					...
					{foreach from=$bi_day item=curr_id}
					  <option value="{$curr_id}">{$curr_id}</option>
					{/foreach}
				</select>
				...
				<select id="_usr_bi-bi_day" name="_usr_bi-bi_month" class="form-control">
					...
					{foreach from=$bi_month item=curr_id}
					  <option value="{$curr_id}">{$curr_id}</option>
					{/foreach}
				</select>
			</div><div class="col-sm-4">
				<select id="_usr_bi-bi_day" name="_usr_bi-bi_year" class="form-control">
					...
					{foreach from=$bi_year item=curr_id}
					  <option value="{$curr_id}">{$curr_id}</option>
					{/foreach}
				</select>
...
<form role="form" class="form-horizontal text-center" action="{$login_link_action}" method="post">
...
<a href="{$register_link}" class="button button-white">Crea una cuenta</a>
</div>
...
<div class="choseoptions"><ul>
	...
	<li class="active"><a href="{$link}cuenta/editar">Editar perfil</a></li>
</ul></div>
</div>
...
if($uc!=false){
	echo '<div class="container"><span rel="notify" data-notification-message="Tu foto de perfil ha sido cambiada!" data-notification-attributes="true">
			<a href="{$link}cuenta" class="btn btn-default">volver</a>
		</span></div><span class="clearfix"></span>';
		echo '{literal}<script>setTimeout(function(){window.location.reload();},1500);</script>{/literal}';
}else{
